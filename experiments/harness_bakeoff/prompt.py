...
def build_prompt(icp: dict[str, Any], max_companies: int | None = None) -> str:
    ...
    return (
        ...
        "- Every submitted company needs a verified canonical company_linkedin; a United States HQ needs evidence-backed "
        "state. linkedin_profile_evidence fields stand alone: identity needs current matching URL/title, never generic "
        "login/sign-up; missing employee_count/quote needs other current proof; listed_headquarters proves only its "
        "literal label. Stored URL/geography proves nothing; omit the company.\n"
